...
fn uid() -> String {
    format!("test_{}", Uuid::new_v4().simple().to_string()[..8].to_string())
}
...
async fn test_set_active_branch_changes_table() {
    ...
    store.register_branch(&user, &branch_name, &table_name).await.expect("register");
    store.set_active_branch(&user, &branch_name).await.expect("set_active");
...
async fn test_active_table_fallback_when_branch_deleted() {
    ...
    store.deregister_branch(&user, &branch_name).await.expect("deregister");
...
async fn test_list_branches() {
    ...
    store.register_branch(&user, &b1, &format!("tbl_{b1}")).await.expect("reg b1");
    store.register_branch(&user, &b2, &format!("tbl_{b2}")).await.expect("reg b2");
...
async fn test_insert_into_branch_table() {
    ...
    let rows = store.list_active_from(&branch_table, &user, 10).await.expect("list branch");
    ...
    let main_rows = store.list_active_from("mem_memories", &user, 10).await.expect("list main");
...
async fn test_merge_branch_into_main() {
    ...
    store.insert_into(&branch_table, &m1).await.expect("insert m1");
    store.insert_into(&branch_table, &m2).await.expect("insert m2");
    ...
    sqlx::query(&sql)
        ...
        .await
        .expect("merge again");
    let main_rows2 = store.list_active_from("mem_memories", &user, 10).await.expect("list main 2");
...
async fn test_full_branch_workflow() {
    ...
    store.insert_into(&active, &branch_mem).await.expect("insert branch");
    ...
    // 5. Checkout main
    store.set_active_branch(&user, "main").await.expect("checkout main");
    let active = store.active_table(&user).await.expect("active_table");
    ...
    sqlx::query(&sql).bind(&user).execute(store.pool()).await.expect("merge");

    let main_rows = store.list_active_from("mem_memories", &user, 10).await.expect("list main after merge");
    assert_eq!(main_rows.len(), 2);
    println!("✅ full workflow: main has {} memories after merge", main_rows.len());
...
async fn test_merge_not_insert_ignore_select_star() {
    ...
    let res = sqlx::query(&merge_sql)
        ...
        .await
        .expect("merge");
    assert_eq!(res.rows_affected(), 1, "merge must insert the row with embedding");

    // Verify it landed in main
    let rows = store.list_active_from("mem_memories", &user, 10).await.expect("list");
    assert_eq!(rows.len(), 1);
    ...
    let broken_sql = format!(
        "INSERT IGNORE INTO mem_memories SELECT * FROM {branch_table} WHERE user_id = ?"
    );
    sqlx::query("DELETE FROM mem_memories WHERE user_id = ?")
        .bind(&user)
        .execute(store.pool())
    ...
    println!("ℹ️  INSERT IGNORE SELECT * rowcount={} (correct fix uses NOT EXISTS instead)",
