...
fn uid() -> String { format!("br_{}", &Uuid::new_v4().simple().to_string()[..8]) }
fn bname(suffix: &str) -> String { format!("b{}_{suffix}", &Uuid::new_v4().simple().to_string()[..6]) }
...
async fn store_mem(content: &str, svc: &Arc<MemoryService>, uid: &str) {
    memoria_mcp::tools::call("memory_store", json!({"content": content}), svc, uid).await.expect("store");
}
...
async fn test_diff_before_merge() {
    ...
    assert!(t.contains("branch-only A") && t.contains("branch-only B"),
...
async fn test_cannot_delete_main() {
    let (svc, git, uid) = setup().await;
    let r = gc("memory_branch_delete", json!({"name": "main"}), &git, &svc, &uid).await;
...
async fn test_delete_nonexistent_branch() {
    let (svc, git, uid) = setup().await;
    let r = gc("memory_branch_delete", json!({"name": "no_such_branch_xyz"}), &git, &svc, &uid).await;
...
async fn test_checkout_nonexistent_branch() {
    ...
    let result = memoria_mcp::git_tools::call(
        "memory_checkout", json!({"name": "ghost_branch"}), &git, &svc, &uid
    ).await;
...
async fn test_duplicate_branch_name_rejected() {
    ...
    assert!(text(&r2).contains("already exists"), "should reject reuse of deleted name, got: {}", text(&r2));
...
async fn test_active_branch_resets_on_delete() {
    ...
    let active = sql.active_table(&uid).await.unwrap();
    assert_eq!(active, "mem_memories", "should reset to main after branch delete");
...
async fn test_multiuser_branch_isolation() {
    ...
    gc("memory_branch", json!({"name": branch_a}), &git, &svc, &uid_a).await;
    gc("memory_checkout", json!({"name": branch_a}), &git, &svc, &uid_a).await;
    store_mem("A branch memory", &svc, &uid_a).await;

    // User B creates their own branch
    store_mem("B main", &svc, &uid_b).await;
    gc("memory_branch", json!({"name": branch_b}), &git, &svc, &uid_b).await;
    gc("memory_checkout", json!({"name": branch_b}), &git, &svc, &uid_b).await;
    ...
    assert!(!text(&r_a).contains(&branch_b), "A should not see B's branch");
    ...
    println!("✅ multiuser isolation: A sees {} memories, B's data not visible", a_mems.len());

    // Cleanup
    gc("memory_checkout", json!({"name": "main"}), &git, &svc, &uid_a).await;
    gc("memory_checkout", json!({"name": "main"}), &git, &svc, &uid_b).await;
    gc("memory_branch_delete", json!({"name": branch_a}), &git, &svc, &uid_a).await;
    gc("memory_branch_delete", json!({"name": branch_b}), &git, &svc, &uid_b).await;
}
...
async fn test_branches_list_shows_active_marker() {
    ...
    assert!(b1_line.contains("← active"), "b1 should be active, got: {b1_line}");
    println!("✅ branches list with active marker:\n{t}");

    gc("memory_checkout", json!({"name": "main"}), &git, &svc, &uid).await;
    gc("memory_branch_delete", json!({"name": b1}), &git, &svc, &uid).await;
    gc("memory_branch_delete", json!({"name": b2}), &git, &svc, &uid).await;
}
...
async fn test_merge_idempotent() {
    ...
    assert!(text(&r2).contains("0 new"), "second merge should be 0: {}", text(&r2));
...
async fn test_merge_nonexistent_branch_errors() {
    let (svc, git, uid) = setup().await;
    let result = memoria_mcp::git_tools::call(
        "memory_merge", json!({"source": "ghost_branch_xyz"}), &git, &svc, &uid
    ).await;
...
async fn test_diff_nonexistent_branch_errors() {
    let (svc, git, uid) = setup().await;
    let result = memoria_mcp::git_tools::call(
        "memory_diff", json!({"source": "ghost_xyz"}), &git, &svc, &uid
    ).await;
    assert!(result.is_err(), "should error");
...
async fn test_branch_name_sanitization() {
    ...
    let r = gc("memory_branch", json!({"name": "my-branch name!"}), &git, &svc, &uid).await;
    // Either created or rejected — must not panic
    println!("✅ special chars branch name: {}", text(&r));
    // Cleanup if created
    gc("memory_branch_delete", json!({"name": "my-branch name!"}), &git, &svc, &uid).await;
}
...
async fn test_diff_fields_complete() {
    ...
    memoria_mcp::tools::call("memory_store",
        json!({"content": "profile memory on branch", "memory_type": "profile"}),
        &svc, &uid).await.expect("store");
...
async fn test_diff_limit_truncation() {
    ...
    let r = gc("memory_diff", json!({"source": branch, "limit": 2}), &git, &svc, &uid).await;
    let t = text(&r);
    assert!(t.contains("showing 2/5") || t.contains("showing 2/"), "expected truncation note, got: {t}");
