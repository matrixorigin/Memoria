...
#[derive(Default)]
struct MockStore { memories: Mutex<Vec<Memory>> }

#[async_trait]
impl MemoryStore for MockStore {
    async fn insert(&self, m: &Memory) -> Result<(), MemoriaError> {
        self.memories.lock().unwrap().push(m.clone()); Ok(())
    }
    async fn get(&self, id: &str) -> Result<Option<Memory>, MemoriaError> {
        Ok(self.memories.lock().unwrap().iter().find(|m| m.memory_id == id && m.is_active).cloned())
    }
    async fn update(&self, memory: &Memory) -> Result<(), MemoriaError> {
        let mut s = self.memories.lock().unwrap();
        if let Some(m) = s.iter_mut().find(|m| m.memory_id == memory.memory_id) { *m = memory.clone(); }
        Ok(())
    }
    async fn soft_delete(&self, id: &str) -> Result<(), MemoriaError> {
        let mut s = self.memories.lock().unwrap();
        if let Some(m) = s.iter_mut().find(|m| m.memory_id == id) { m.is_active = false; }
        Ok(())
    }
    ...
    async fn search_fulltext(&self, user_id: &str, q: &str, limit: i64) -> Result<Vec<Memory>, MemoriaError> {
        Ok(self.memories.lock().unwrap().iter()
            .filter(|m| m.user_id == user_id && m.is_active && m.content.contains(q))
            .take(limit as usize).cloned().collect())
    }
    async fn search_vector(&self, _: &str, _: &[f32], _: i64) -> Result<Vec<Memory>, MemoriaError> { Ok(vec![]) }
}

struct MockEmbedder;
#[async_trait]
impl EmbeddingProvider for MockEmbedder {
    async fn embed(&self, _: &str) -> Result<Vec<f32>, MemoriaError> { Ok(vec![0.1; 4]) }
    fn dimension(&self) -> usize { 4 }
}

fn make_service() -> Arc<MemoryService> {
    Arc::new(MemoryService::new(Arc::new(MockStore::default()), Some(Arc::new(MockEmbedder))))
}
...
async fn test_tool_memory_retrieve_finds() {
    let svc = make_service();
    // Store first
    memoria_mcp::tools::call("memory_store", json!({"content": "rust programming"}), &svc, "u1").await.unwrap();
    // Retrieve
    let result = memoria_mcp::tools::call(
        "memory_retrieve",
        json!({"query": "rust"}),
...
async fn test_tool_memory_correct() {
    let svc = make_service();
    let stored = memoria_mcp::tools::call("memory_store", json!({"content": "old"}), &svc, "u1").await.unwrap();
    let text = stored["content"][0]["text"].as_str().unwrap();
    // Extract memory_id from "Stored memory <id>: old"
    let memory_id = text.split_whitespace().nth(2).unwrap().trim_end_matches(':');
...
async fn test_tool_memory_purge() {
    let svc = make_service();
    let stored = memoria_mcp::tools::call("memory_store", json!({"content": "to delete"}), &svc, "u1").await.unwrap();
    let text = stored["content"][0]["text"].as_str().unwrap();
    let memory_id = text.split_whitespace().nth(2).unwrap().trim_end_matches(':');
